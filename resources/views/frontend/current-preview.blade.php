...
<x-front-header-component />
   <div class="Projects">
    ...
    @php
        ...
        $updatedHtmlContent = str_replace('/animations.css', '/animations.csss', $htmlContent);
    @endphp
...
<script>
    ...
    $(window).on('load', function() {
        ...
        if (typeof jQuery === 'undefined') {
            // If not included, add jQuery script
            let jqueryScript = document.createElement('script');
            jqueryScript.src = "https://code.jquery.com/jquery-3.5.1.min.js";
            document.head.appendChild(jqueryScript);

            // After jQuery is loaded, load Bootstrap
            ...
            // If jQuery is present, check if Bootstrap is loaded
            if (typeof $.fn.modal === 'undefined') {
                // If Bootstrap is not loaded, add Bootstrap script
                let bootstrapScript = document.createElement('script');
                bootstrapScript.src = "https://stackpath.bootstrapcdn.com/bootstrap/4.5.2/js/bootstrap.bundle.min.js";
                document.head.appendChild(bootstrapScript);
            }
        ...
        $('*').each(function() {
            ...
            const bottom = $element.css('bottom');

            // Check if the element has position: fixed and top: 0
            if (position === 'fixed' && top === '0px') {
            $element.addClass('cmsTop');
            }

            // Check if the element has position: fixed and bottom: 0
            if (position === 'fixed' && bottom === '0px' && !$element.hasClass('preview_sect')) {
            $element.addClass('cmsBottom');
            }
        });
...
function adjustFixedElementPosition() {
    const $topElement = $('.cmsTop'); 
    const $bottomElement = $('.cmsBottom'); 
    const navbarOuterHeight = $('#navbar').outerHeight();
    const bottomOuterHeight = $('.preview_sect').outerHeight();
    $topElement.css('top', navbarOuterHeight+"px");
    $bottomElement.css('bottom', bottomOuterHeight+"px");
}

// Adjust on resize and scroll
$(window).on('resize scroll', adjustFixedElementPosition);
